...
def _load_dataset(*args, **kwargs):
    if hf_load_dataset is None:
        raise ImportError(
            "Hugging Face 'datasets' is unavailable or shadowed by a local "
            "'datasets' directory. Use local CSVs with --root_path/--data_path, "
            "rename the local folder, or install the datasets package."
        )
...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        if os.path.exists(local_fp):
            ...
            ds = _load_dataset(HUGGINGFACE_REPO, name=cfg_name)
...
class PSMSegLoader(Dataset):
    def __init__(self, args, root_path, win_size, step=1, flag="train"):
        ...
        if all(os.path.exists(p) for p in [train_path, test_path, label_path]):
            ...
            ds_data = _load_dataset(HUGGINGFACE_REPO, name="PSM-data")
            ds_label = _load_dataset(HUGGINGFACE_REPO, name="PSM-label")
...
class SWATSegLoader(Dataset):
    def __init__(self, args, root_path, win_size, step=1, flag="train"):
        ...
        if all(os.path.exists(p) for p in [train2_path, test_path]):
            ...
            ds = _load_dataset(HUGGINGFACE_REPO, name="SWaT")
