...
EXPS = [
    ("G0", ["--decomp_gate_force", "1"]),
    ("G1", ["--decomp_gate_force", "0"]),
    ("G2", ["--decomp_gate", "1", "--decomp_gate_bias_init", "2"]),
]


def run_exp(exp_id, extra):
    model_id = f"DGmix_{DATASET}_96_96_DG_{exp_id}"
    log_id = f"{DATASET}_DG_{exp_id}"
