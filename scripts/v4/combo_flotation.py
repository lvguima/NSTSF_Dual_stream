...
EXPS = [
    ("C1", ["--decomp_gate", "1", "--decomp_gate_bias_init", "2", "--factor_mix", "0"]),
    ("C2", ["--decomp_gate", "0", "--factor_mix", "1", "--factor_alpha_init", "-2"]),
    ("C3", ["--decomp_gate", "1", "--decomp_gate_bias_init", "2", "--factor_mix", "1", "--factor_alpha_init", "-2"]),
]


def run_exp(exp_id, extra):
    model_id = f"DGmix_{DATASET}_96_96_CMB_{exp_id}"
    log_id = f"{DATASET}_CMB_{exp_id}"
